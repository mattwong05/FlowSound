...
if [[ ! "$VERSION" =~ '^[0-9]+\.[0-9]+\.[0-9]+$' ]]; then
    echo "VERSION must use MAJOR.MINOR.PATCH SemVer format; found '$VERSION'." >&2
    exit 1
fi
...
sed "s/__FLOWSOUND_VERSION__/$VERSION/g" "packaging/Info.plist" > "$CONTENTS_DIR/Info.plist"
