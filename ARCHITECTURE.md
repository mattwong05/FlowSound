...
Owns audio activity detection. The current implementation uses `ManualAudioActivityMonitor` for end-to-end service testing. The next implementation step is a Core Audio process tap-backed monitor behind the same `AudioActivityMonitor` protocol.
...
Generated by `scripts/generate-logo-assets.swift` from `FlowSound-iCon.png`.
...
The menu bar icon uses the transparent template asset so macOS can tint it for light and dark menu bars.
